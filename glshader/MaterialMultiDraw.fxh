...
layout (location = 5) uniform vec4 lightPos[iLightPointNum];
layout (location = 7) uniform vec4 lightEmission[iLightPointNum];
#else
layout (location = 5) uniform vec4 lightPos[iLightPointBallsNum];
layout (location = 7) uniform vec4 lightEmission[iLightPointBallsNum];
#endif
layout (location = 9) uniform int lightSources;

layout (location = 10) uniform vec4 cAmbient_LightRange;// = vec4(0.0,0.0,0.0, 0.0); //!! remove completely, just rely on envmap/IBL?

layout (location = 11) uniform vec2 fenvEmissionScale_TexWidth;
...
vec3 DoPointLight(vec3 pos, vec3 N, vec3 V, vec3 diffuse, vec3 glossy, float edge, int i, bool is_metal, vec4 Roughness_WrapL_Edge_Thickness, vec4 fDisableLighting_top_below) 
{ 
...
vec3 DoEnvmapDiffuse(vec3 N, vec3 diffuse, bool hdrEnvTextures)
{
